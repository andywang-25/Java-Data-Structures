...
public class GraphRunner
{
	public static void main( String[] args ) throws IOException
	{
		...
		for(int x=0; x<howManyTimes; x++)
		{
			...
			Graph test = new Graph(input); //create graph out of all those numbers 
			...
			if (test.contains(start) && test.contains(stop)) //check if test even contains both of the numbers 
				test.check(input.substring(0,1), input.substring(1,2), "");  //then check if they are CONNECTED to each other 
